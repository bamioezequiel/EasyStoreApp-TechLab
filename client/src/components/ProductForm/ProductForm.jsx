...
export default function ProductForm({ onSuccess, initialData = {}, onCancel }) {
  ...
  return (
    <form className={`${styles.form} ${animate ? styles.animateIn : ''}`} onSubmit={handleSubmit}>
      ...
      <textarea placeholder="Descripción" value={description} onChange={e => setDescription(e.target.value)} rows={3} required />

      <input type="url" placeholder="URL de la imagen" value={imageUrl} onChange={e => setImageUrl(e.target.value)} required />
